...
// CODER
{
	Coder.processLineE = function(line, element){
		if(!element)
			return null;
		...
		if(!(line instanceof Element))
			return null;
		let codeIns = line.getElementsByClassName('codeIn');
		codeIns = Array.from(codeIns);
		codeIns.forEach((e)=>{
			e.style.cursor = "pointer";
			e.onclick = ()=>{
				element.value = e.innerText;
				element.onkeyup({target: element, which:13, keyCode: 13});
			};
		});
		return true;
	}

	Coder.processLinesE = function(lines, element){
		if(!element || !lines)
			return null;
		if(!Array.isArray(lines))
			return null;
		lines.forEach(e=>this.processLineE(e, element));
	}

	Coder.processLinesC = function(lines, callback){
		if(!callback || !lines)
			return null;
		...
		if(!Array.isArray(lines))
			return null;
		lines.forEach(e=>this.processLineC(e, callback));
	}

	Coder.processLineC = function(line, callback){
		if(!callback)
			return null;
		if(typeof callback !== 'function')
			return null;
